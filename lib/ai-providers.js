 ...
 * Supports multiple free AI providers with fallback logic
 */

import axios from 'axios';

/**
 * Call Groq API (Fast, Free, OpenAI-compatible)
 */
...
/**
 * Call Google Gemini API (High Quality)
 */
...
/**
 * Call OpenRouter API (Aggregator with free models)
 */
...
export async function callAIWithFallback(messages, systemPrompt, maxTokens, temperature = 0.7, options = {}) {
  ...
  // Build message array for OpenAI-compatible APIs
  const messageArray = [
    ...(systemPrompt ? [{ role: 'system', content: systemPrompt }] : []),
    ...messages
  ];

  // Provider list with priority order
  const providers = [
    {
      name: 'groq',
