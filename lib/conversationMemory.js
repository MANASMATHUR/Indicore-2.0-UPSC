...
export function buildConversationMemoryPrompt(summaries = [], limit = 8) {
  ...
  const sections = sorted.map((entry, idx) => {
    ...
    // Add recency indicator for very recent conversations
    const timestamp = new Date(entry.timestamp);
    const hoursSince = (Date.now() - timestamp.getTime()) / (1000 * 60 * 60);
    const recencyTag = hoursSince < 24 ? ' [Recent]' : '';

    return `${idx + 1}. ${title}${recencyTag}\n   Summary: ${summary}${formattedPoints}`;
  });
...
export async function saveConversationMemory({
  ...
  try {
    ...
    user.profile.conversationSummaries = filtered.slice(0, 15); // Increased from 8 to 15
