...
export function cleanAIResponse(response) {
  ...
  // Check if this is a PYQ response from database (has specific formatting)
  const isPyqDbResponse = /^Previous\s+Year\s+Questions|Year\s+\d{4}\s+\(\d+\s+question/i.test(cleaned);
  
  // For PYQ database responses, do minimal cleaning - just remove citations
  if (isPyqDbResponse) {
    // Only remove citation patterns, preserve everything else
    cleaned = cleaned.replace(/\[\d+(?:\s*,\s*\d+)*\]/g, '');
    cleaned = cleaned.replace(/\([A-Z][a-zA-Z\s]+\):\s*(?![A-Z])/g, '');
  ...
  // Remove Perplexity-specific citation patterns first
  // Patterns like "(Middle East Eye):", "(Countercurrents):", "(Source Name):"
  // But be careful not to remove content in parentheses that's part of the text
  // Also preserve PYQ paper names like [GS Paper 1] or [Prelims]
  // Only remove if it's followed by a colon and looks like a citation
  cleaned = cleaned.replace(/\([A-Z][a-zA-Z\s]{3,}\):\s*(?![A-Z])/g, '');
  
  // Remove "From result" patterns (Perplexity citation format)
  cleaned = cleaned.replace(/From\s+result\s*\([^)]*\)\s*[:.]?\s*/gi, '');
  cleaned = cleaned.replace(/From\s+result\s*[:.]?\s*/gi, '');
  cleaned = cleaned.replace(/From\s+result[^.!?]*/gi, '');
  
  // Remove incomplete citations like "(reigned", "(Source" at end of line
  cleaned = cleaned.replace(/\([^)]*$/gm, '');
  cleaned = cleaned.replace(/\(reigned[^)]*\)/gi, '');
  
  // Remove patterns like "(reigned (Middle East Eye):" - nested citations
  cleaned = cleaned.replace(/\([^)]*\([^)]*\)[^)]*\)/g, '');
  
  // Remove broken citation patterns like "(Source Name): text" that appear mid-sentence
  cleaned = cleaned.replace(/\s*\([A-Z][a-zA-Z\s]{2,}\)\s*:\s*(?=\s*[a-z])/g, ' ');
  ...
  cleaned = cleaned.replace(/([a-z])\s*\([A-Z][a-zA-Z\s]{3,}\)\s*:\s*([a-z])/gi, '$1 $2');
  ...
  // Replace multiple spaces with single space, but preserve newlines for paragraphs
  cleaned = cleaned.replace(/[ \t]+/g, ' ');
...
export function validateAndCleanResponse(response, minLength = 20) {
  ...
  if (isGarbledResponse(cleaned)) {
    // Only reject if it's clearly garbled AND very short
    if (cleaned.length < 50) {
      return null;
    }
    // For PYQ responses, be more lenient - they may have list formatting
    if (isPyqResponse && cleaned.length > 100) {
      // Allow PYQ responses even if they have some garbled patterns
      return cleaned;
    }
