...
export function validateAndCleanResponse(response, minLength = 20) {
  ...
  // This handles cases like "Hi!" (3 chars) or "Akbar was a Mughal emperor." (30 chars)
  if (cleaned.length >= 3 && cleaned.length < 20 && minLength <= 15) {
    const looksComplete = /[.!?]$/.test(cleaned.trim()) && 
                         ...
                         !isGarbledResponse(cleaned);
    if (looksComplete) {
      return cleaned;
    }
  }
  
  // For extremely short responses (3-5 chars), accept if they're not garbled
  // This handles greetings like "Hi!" or "Hey!"
  if (cleaned.length >= 3 && cleaned.length <= 5 && minLength <= 5) {
    if (!isGarbledResponse(cleaned) && /[.!?]?$/.test(cleaned.trim())) {
      return cleaned;
    }
  }
