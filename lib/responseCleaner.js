...
function ensureMarkdownSpacing(text) {
  ...
  // Ensure blank line after headings or bold section titles
  formatted = formatted.replace(
    /((?:\*\*[^\n]+\*\*)|(?:#{1,6}\s+[^\n]+))\n(?!\n|[-*•]\s|\d+\.\s)/g,
    '$1\n\n'
  );
  // Ensure blank line before new major sections (bold heading or numbered list)
  formatted = formatted.replace(
    /([^\n])\n(?=(?:\*\*[^\n]+\*\*|#{1,6}\s+[^\n]+|\d+\.\s))/g,
    '$1\n\n'
  );
  // Provide breathing room before bullet blocks
  formatted = formatted.replace(/([^\n])\n(?=(?:[-*•]\s))/g, '$1\n\n');
  // Reduce any extreme spacing down to double newlines
  formatted = formatted.replace(/\n{4,}/g, '\n\n');
  return formatted;
}
