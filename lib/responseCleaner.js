...
export function cleanAIResponse(response) {
  ...
  if (isPyqDbResponse) {
    ...
    // Only normalize excessive newlines (4+ to 2), don't remove all
  ...
  // For markdown content, preserve structure more carefully
  if (finalHasMarkdown) {
    // Only normalize spaces/tabs within lines, preserve all newlines for markdown
    cleaned = cleaned.replace(/[ \t]+/g, ' ');
    // Normalize excessive blank lines (5+ to 2), but preserve markdown structure
    cleaned = cleaned.replace(/\n{5,}/g, '\n\n');
    // Don't normalize triple newlines in markdown - they might be intentional
    return cleaned.trim();
  }
  
  // For non-markdown content, normalize whitespace
  cleaned = cleaned.replace(/[ \t]+/g, ' '); // Normalize spaces/tabs within lines
  // Normalize excessive blank lines (4+ to 2)
  cleaned = cleaned.replace(/\n{4,}/g, '\n\n\n');
  // Normalize triple newlines to double (paragraph break)
  cleaned = cleaned.replace(/\n{3}/g, '\n\n');
...
export function validateAndCleanResponse(response, minLength = 20) {
  ...
  if (cleaned.length >= 3 && cleaned.length <= 5 && minLength <= 5) {
    ...
    if (!hasGarbledPatterns && /[.!?]?$/.test(cleaned.trim())) {
      return cleaned;
    }
  }
  ...
  // This prevents rejecting valid short responses
  if (cleaned.length < 20) {
    ...
    // If no garbled patterns, accept it (effectiveMinLength check already validated minimum length)
    // This prevents falling through to isGarbledResponse() which would reject based on length
    return cleaned;
