...
export function cleanAIResponse(response) {
  ...
  if (finalHasMarkdown) {
    ...
    cleaned = cleaned.replace(/[ \t]+/g, ' ');
    // Reduce excessive blank lines (4+ to 2) to preserve markdown structure
    // Markdown needs blank lines between sections, so we only reduce truly excessive spacing
    cleaned = cleaned.replace(/\n{4,}/g, '\n\n');
    return cleaned.trim();
  }
...
function ensureMarkdownSpacing(text) {
  ...
  // First, reduce excessive spacing (4+ newlines to 2) to normalize
  formatted = formatted.replace(/\n{4,}/g, '\n\n');
  
  // Ensure blank line after headings or bold section titles (if content follows and no blank line exists)
  // Don't add blank line if next thing is already a list item (bullet or numbered)
  formatted = formatted.replace(
    /((?:\*\*[^\n]+\*\*)|(?:#{1,6}\s+[^\n]+))\n(?!\n|[-*•]\s|\d+\.\s)/g,
    '$1\n\n'
  );
  ...
  // Only add if there's content before and no blank line already exists
  formatted = formatted.replace(
    /([^\n])\n(?=(?:\*\*[^\n]+\*\*|#{1,6}\s+[^\n]+|\d+\.\s))/g,
    '$1\n\n'
  );
