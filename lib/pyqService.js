...
class PyqService {
  ...
  formatResults(items, params, options = {}) {
    ...
    for (const q of sortedItems) {
      ...
      // Use bold for paper name - brackets inside bold text are fine in markdown
      // Format: **[Paper Name]** - this renders as bold brackets with paper name
      let label = `**[${paperName}]** ${questionText}`;
      
      // Add topic tags if they provide additional context (use italic for subtle emphasis)
