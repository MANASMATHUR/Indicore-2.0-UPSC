...
class PyqService {
  ...
  formatResults(items, params, options = {}) {
    ...
    for (const q of sortedItems) {
      ...
      // Build question label with consistent formatting
      let label = `[${paperName}] ${questionText}`;
      
      if (topicTagsFormatted) {
        label += ` (${topicTagsFormatted})`;
      }
      
      // Add status markers at the end
      if (isUnverified) {
        label += ' ⚠️';
    ...
    for (const year of sortedYears) {
      ...
      // Single blank line between years
      lines.push('');
    }
    ...
    if (verifiedCount > 0 && unverifiedCount > 0) {
      ...
      lines.push(`• All ${unverifiedCount} questions unverified - please verify before use ⚠️`);
    }
    lines.push('');
    
    // Analysis feature info
    lines.push('💡 Analysis Feature');
    lines.push('Questions marked with 📊 have detailed analysis available.');
  ...
  async enhanceWithAIFormatting(content, params) {
    ...
    // CRITICAL: If content already has proper formatting with newlines, skip AI formatting if disabled
    ...
    const systemPrompt = `You are an expert formatting assistant for UPSC Previous Year Questions (PYQs). Transform messy question data into clean, professional, student-friendly format.

CRITICAL FORMATTING RULES:
1. PRESERVE ALL QUESTION TEXT EXACTLY - Never modify, summarize, or truncate questions
2. ORGANIZE by year (newest first) with clear headers: "Year YYYY · X questions"
3. FORMAT each question on ONE line: "1. [Paper] Question text (Topic tags) ✅/⚠️ 📊"
4. REMOVE duplicates, artifacts, and metadata noise
5. USE consistent spacing: ONE blank line between sections, NO blank lines between questions in same year
6. PRESERVE mathematical notation exactly ($...$ and $$...$$)
7. KEEP it simple: plain text, minimal formatting, clear structure
8. CRITICAL: PRESERVE ALL NEWLINES - Use actual line breaks (\n) between sections. Never remove newlines or join separate lines together. Each section must be on its own line.
...
1. [Paper] Question text (Topic tags) ✅
2. [Paper] Question text (Topic tags) ⚠️
3. [Paper] Question text ✅ 📊

Year [YYYY] · [Count] questions

1. [Paper] Question text ✅
...
...
💡 Analysis Feature
Questions marked with 📊 have detailed analysis available.
• Say "analyze question [number]" for a specific question
• Or paste the question text and say "analyze this question"

STRICT PROHIBITIONS:
- NO markdown headers (###, ##) - use plain "Year YYYY"
- NO bold text (**text**) - use plain text only
- NO blank lines between questions in same year
- NO modification of question text
- NO commentary within questions
- NO removal of markers (✅/⚠️/📊)
- MUST clean and reorganize - don't return messy format`;
    ...
    try {
      ...
      if (aiContent && aiContent.trim().length >= 100) {
        ...
        // CRITICAL: Preserve newlines - they are essential for PYQ formatting
        // Only remove blank lines between consecutive questions in same year
  ...
  async search(userMsg, context = null, language = 'en') {
    try {
      ...
      // COMPLETELY DISABLE AI FORMATTING - it keeps removing newlines
      // Use original formatted content which has proper newlines
      let polishedContent = formatted;
      
      // AI formatting is DISABLED by default - only enable if explicitly set
      // AND only if it preserves newlines
      if (process.env.ENABLE_PYQ_AI_FORMATTING === 'true' && formattedHasNewlines) {
        try {
