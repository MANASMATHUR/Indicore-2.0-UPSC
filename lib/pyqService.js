...
import { TOPIC_HIERARCHY } from './topicHierarchy';
...
class PyqService {
  ...
  detectExamCode(userMsg, language) {
    const patterns = [
      ...
      { test: /\b(all\s+)?(pcs|state\s*psc|civil\s*services)\b/i, code: 'ALL_PCS' }
    ];
    ...
    // Default to ALL_PCS (UPSC + State PCS) to provide comprehensive results
    return 'ALL_PCS';
  }
  ...
  parseQuery(userMsg, language) {
    ...
    if (rangeMatch) {
      ...
      if (year >= 1979 && year <= new Date().getFullYear()) {
        fromYear = year;
  ...
  _processTheme(theme, fromYear, toYear, userMsg, language, levelHint = '') {
    ...
    // Intensive Sub-topic Detection
    let detectedSubTopic = '';
    let parentTopic = '';
    let detectedSubject = '';

    for (const [subjectKey, subjectData] of Object.entries(TOPIC_HIERARCHY)) {
      if (lowerTheme.includes(subjectKey.replace('_', ' ')) || lowerTheme.includes(subjectData.label.toLowerCase())) {
        detectedSubject = subjectData.label;
      }

      for (const [topicKey, topicData] of Object.entries(subjectData.topics)) {
        if (lowerTheme.includes(topicKey.replace('_', ' ')) || lowerTheme.includes(topicData.label.toLowerCase())) {
          parentTopic = topicData.label;
          detectedSubject = subjectData.label;
        }

        for (const sub of topicData.subTopics) {
          if (lowerTheme.includes(sub.toLowerCase())) {
            detectedSubTopic = sub;
            parentTopic = topicData.label;
            detectedSubject = subjectData.label;
            break;
          }
        }
        if (detectedSubTopic) break;
      }
      if (detectedSubTopic) break;
    }
  ...
  buildFilter(params) {
    ...
    if (examCode === 'ALL_PCS') {
      // Search across UPSC and all State PSCs
      filter.exam = { $in: ['UPSC', 'PCS', 'MPSC', 'TNPSC', 'BPSC', 'UPPSC', 'MPPSC', 'RAS', 'RPSC', 'GPSC', 'KPSC', 'WBPSC', 'PPSC', 'OPSC', 'APSC', 'APPSC', 'TSPSC', 'HPSC', 'JKPSC', 'KERALA PSC', 'GOA PSC'] };
    } else {
      filter.exam = new RegExp(`^${examCode}$`, 'i');
    }
  ...
  async queryDatabase(filter, theme, options = {}) {
    ...
    try {
      ...
      if (theme) {
        ...
        // PRIORITY 5: Search in subTopic field (NEW)
        if (combinedRegex) {
          searchConditions.push({ subTopic: { $regex: combinedRegex } });
        }
  ...
  formatResults(items, params, options = {}) {
    ...
    for (const q of sortedItems) {
      const year = q.year || 0;
      if (year < 1979 || year > new Date().getFullYear()) continue;
