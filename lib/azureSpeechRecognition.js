  ...
  async initialize() {
    try {
      ...
      if (!window.SpeechSDK) {
        ...
        if (!document.getElementById('azure-speech-sdk')) {
          ...
          await new Promise((resolve, reject) => {
            script.onload = () => {
              // Wait for SpeechSDK to be available
              ...
              setTimeout(() => {
                clearInterval(checkSDK);
                reject(new Error('Azure Speech SDK failed to load'));
              }, 10000);
            };
          ...
          // Script already exists, wait for SDK to load
          await new Promise((resolve, reject) => {
            const checkSDK = setInterval(() => {
              if (window.SpeechSDK) {
                clearInterval(checkSDK);
      ...
      console.warn('Azure Speech Recognition not available, falling back to browser recognition:', error);
      this.isAzureAvailable = false;
      return false;
    }
  }
