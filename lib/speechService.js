...
class SpeechService {
  ...
  cleanTextForSpeech(text, language = 'en') {
    ...
    // Check if this is an Indian language (handle both short codes and full codes)
    const shortLang = language?.split('-')[0] || language;
    const isIndianLanguage = ['hi', 'mr', 'ta', 'bn', 'pa', 'gu', 'te', 'ml', 'kn'].includes(shortLang);
    ...
    if (hasIndianScript || isIndianLanguage) {
      // For Indian languages, use careful cleaning to preserve natural flow and script
      ...
      // For Indian languages, preserve newlines as sentence breaks but don't convert to periods
      // This helps maintain natural flow in Indian languages
      cleanedText = cleanedText
        .replace(/\n+/g, ' ') // Convert newlines to spaces for speech
        .replace(/([.!?])([^\s])/g, '$1 $2')
        .replace(/([,:;])([^\s])/g, '$1 $2');
  ...
  getAzureVoice(language) {
    ...
    const voiceMap = {
      ...
      'pa': null, // Punjabi voice not available in Azure
  ...
  async speak(text, language = 'en', options = {}) {
    ...
    try {
      ...
      // For Indian languages, try to initialize Azure if not already available
      if (isIndianLanguage && !this.isAzureAvailable) {
        await this.initializeAzure();
      }

      // Wait for voices to be loaded before speaking (for browser fallback)
      if (!this.isAzureAvailable && 'speechSynthesis' in window) {
        await this.waitForVoices();
      }

      const cleanText = this.cleanTextForSpeech(text, shortLang);
      ...
      const optimalParams = this.getSpeechParams(cleanText, shortLang);
      ...
      let result;
      // Prioritize Azure for Indian languages, especially on mobile
      if (this.isAzureAvailable && (isIndianLanguage || isMobile)) {
        try {
          result = await this.speakWithAzure(cleanText, shortLang, speechOptions);
          this.performanceMetrics.successfulRequests++;
          this.updatePerformanceMetrics(startTime);
          return result;
        } catch (azureError) {
          console.warn('Azure speech failed, falling back to browser:', azureError);
          // Fall through to browser synthesis
        }
      }
      
      result = await this.speakWithBrowser(cleanText, shortLang, speechOptions);
      this.performanceMetrics.successfulRequests++;
      this.updatePerformanceMetrics(startTime);
      return result;
  ...
  async speakWithAzure(text, language, options) {
    try {
      // Get short language code
      const shortLang = language?.split('-')[0] || language || 'en';
      const azureVoice = this.getAzureVoice(shortLang);
      
      if (!azureVoice) {
        throw new Error(`No Azure voice available for language: ${shortLang}`);
      }
  ...
  async speakWithBrowser(text, language, options = {}) {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Get short language code (handle both 'mr' and 'mr-IN' formats)
        const shortLang = language?.split('-')[0] || language || 'en';
        const langCode = this.getLanguageCode(shortLang);
        ...
        const speakChunk = () => {
          ...
          const selectVoice = () => {
            ...
            if (voices.length > 0) {
              const selectedVoice = this.selectBestVoice(voices, langCode, shortLang);
              ...
              if (selectedVoice) {
                ...
                utterance.lang = langCode;
                utterance.voice = null; // Let browser choose default for that language
                
                // Only fallback to English if we're on mobile and it's an Indian language
                const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
                const regionalLanguages = ['hi', 'mr', 'ta', 'bn', 'pa', 'gu', 'te', 'ml', 'kn'];
                
                if (isMobile && regionalLanguages.includes(shortLang)) {
                  // On mobile, try to use the language code anyway - some mobile browsers support it
                  // Only fallback if it actually fails
                  console.log(`Using language code ${langCode} for ${shortLang} on mobile`);
                }
              }
              ...
              utterance.onerror = (event) => {
                console.warn(`Speech error for ${langCode}:`, event.error);
                // Don't automatically fallback - let the error propagate
                this.isSpeaking = false;
                reject(new Error(`Speech synthesis failed for ${shortLang}: ${event.error}`));
              };
