...
class SpeechService {
  ...
  getOptimalSpeechParams(text, language) {
    ...
    // Base parameters optimized for natural, clear speech
    let rate = 0.88;
    let pitch = 1.0;
    let volume = 1.0;
    
    // Question intonation - slightly higher pitch for natural questioning tone
    if (hasQuestions) {
      pitch = 1.06;
      rate = 0.87; // Slightly slower for clarity
    }
    
    // Exclamation - expressive but not too fast
    if (hasExclamations) {
      rate = 0.9;
      pitch = 1.03;
    }
    
    // Numbers and technical content - slower for clarity
    if (hasNumbers) {
      rate = 0.82;
    }
    
    // Lists - slower pace for better comprehension
    if (hasLists) {
      rate = 0.85;
    }
    
    // Long text - slightly faster but still clear
    if (isLongText && !isVeryLongText) {
      rate = 0.9;
    }
    
    // Very long text - optimal pace for endurance
    if (isVeryLongText) {
      rate = 0.92;
    }
    
    // Language-specific optimizations for natural tonality
    const languageAdjustments = {
      'en': { rate: 0.88, pitch: 1.0, volume: 1.0 }, // Clear, professional English
      'hi': { rate: 0.82, pitch: 1.0, volume: 1.0 }, // Hindi - slightly slower for clarity
      'mr': { rate: 0.85, pitch: 0.99, volume: 1.0 }, // Marathi - natural rhythm
      'ta': { rate: 0.82, pitch: 1.02, volume: 1.0 }, // Tamil - melodic tone
      'bn': { rate: 0.85, pitch: 1.0, volume: 1.0 }, // Bengali - balanced
      'pa': { rate: 0.88, pitch: 1.0, volume: 1.0 }, // Punjabi - energetic
      'gu': { rate: 0.85, pitch: 0.99, volume: 1.0 }, // Gujarati - smooth
      'te': { rate: 0.82, pitch: 1.0, volume: 1.0 }, // Telugu - clear pronunciation
      'ml': { rate: 0.85, pitch: 1.0, volume: 1.0 }, // Malayalam - natural flow
      'kn': { rate: 0.85, pitch: 1.0, volume: 1.0 }, // Kannada - balanced pace
      'es': { rate: 0.9, pitch: 1.0, volume: 1.0 }  // Spanish - expressive
    };
    
    const langAdjustment = languageAdjustments[language] || languageAdjustments['en'];
    
    // Apply language-specific base, then adjust for context
    rate = langAdjustment.rate;
    pitch = langAdjustment.pitch;
    volume = langAdjustment.volume;
    
    // Fine-tune for specific contexts (override base if needed)
