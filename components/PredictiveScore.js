...
export default function PredictiveScore({ prediction }) {
    ...
    const gaugeColor = isSafe ? 'text-rose-500' : (isRisky ? 'text-red-500' : 'text-orange-500');

    return (
        <Card className="border border-rose-100 dark:border-rose-900/30 bg-white dark:bg-gray-900 shadow-xl shadow-rose-500/5 h-full relative overflow-hidden">
            {/* Glossy overlay effect */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-rose-500/5 to-transparent rounded-full -mr-20 -mt-20 pointer-events-none" />

            <CardHeader className="pb-2 border-b border-rose-50 dark:border-rose-900/10">
            ...
            <CardContent className="pt-6 relative z-10 text-center">
                ...
                <div className="mb-6">
                    ...
                    <div className="flex items-baseline justify-center gap-2">
                        <span className="text-5xl font-extrabold text-rose-600 dark:text-rose-400 tracking-tight">
                            {predictedScore}
                        </span>
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-800">
                    ...
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 overflow-hidden">
                        <div
                            className={`h-2.5 rounded-full transition-all duration-1000 ${isSafe ? 'bg-rose-500' : (isRisky ? 'bg-red-500' : 'bg-orange-500')}`}
