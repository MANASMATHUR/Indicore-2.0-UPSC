...
const ChatMessages = memo(({ messages = [], isLoading = false, messagesEndRef, onRegenerate, onPromptClick }) => {
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3 max-w-3xl mx-auto">
            {[
              ...
              <button
                ...
                className="group px-5 py-4 text-left bg-white/90 dark:bg-slate-800/90 hover:bg-white dark:hover:bg-slate-800 border border-red-200/60 dark:border-slate-700/60 rounded-2xl text-sm font-medium text-red-900 dark:text-slate-200 hover:border-red-400 dark:hover:border-red-600 hover:shadow-xl hover:shadow-red-500/10 dark:hover:shadow-red-500/20 transition-all duration-300 backdrop-blur-xl hover:-translate-y-0.5 active:translate-y-0"
              >
  ...
  return (
    <div className="flex-1 overflow-y-auto messages scroll-smooth" style={{ minHeight: 'calc(100vh - 200px)' }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-4">
...
const MessageItem = memo(({ 
  ...
  const handleCopy = async () => {
    ...
    if (onCopy) onCopy(index);
  };

  return (
    <div 
      className={`message ${sender === 'user' ? 'user' : 'assistant'} animate-fade-in flex items-start gap-3`} 
